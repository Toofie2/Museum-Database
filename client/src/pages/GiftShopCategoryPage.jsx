...
const GiftShopCategoryPage = () => {
  ...
  useEffect(() => {
    const fetchGiftShopCategoryView = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/product_category/${id}`);
        setProductCategory(response.data);

        // Fetch products in category
        const productResponse = await axios.get(`http://localhost:3000/product/product_category/${id}`);
  ...
  if (!productCategory) return <div className="text-center mt-20">Loading...</div>;
      ...
      <div className="mt-20 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 px-8">
        {products.map((p) => (
          <div className="static">
            ...
            <h3 className="text-[16px] font-medium text-black pl-8 mt-2">{p.name}</h3>
            <p className="text-[18px] font-medium text-default-gray pl-8 mt-1">${p.price}</p>
          </div>
        ))}
      </div>
    </div>
