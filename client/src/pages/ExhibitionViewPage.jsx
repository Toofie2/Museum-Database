...
const ExhibitionViewPage = () => {
  ...
  useEffect(() => {
    const fetchExhibitionView = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/exhibition/${id}`);
        setExhibition(response.data);

        // Fetch art pieces where exhibit_id matches
        const artResponse = await axios.get(`http://localhost:3000/art/exhibit/${id}`);
    ...
    <div className="container mx-auto pb-12">
      ...
      <div className="relative flex items-center h-[800px] w-full">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col justify-center pl-4 space-y-2">
          <h1 className="text-8xl font-regular text-white z-10 w-[900px] p-6">{exhibition.name}</h1>
          <p className="text-3xl font-thin text-white z-10 w-[800px] p-6">{exhibition.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-8">
        {artPieces.map((art) => (
          <div key={art.art_id} className="h-[300px] art-item relative rounded-lg overflow-hidden">
            <img
            ...
            <h3 className="text-2xl font-thin text-white absolute bottom-0 left-0 ml-4 mb-4 z-10">{art.art_title}</h3>
          </div>
        ))}
      </div>
    </div>
