...
const CollectionsPage = () => {
  ...
  useEffect(() => {
    const fetchCollections = async () => {
      try {
        const response = await axios.get('http://localhost:3000/collection'); // Adjust API endpoint
    ...
    <div className="container mx-auto p-0">
      ...
      <div className="relative flex items-center h-[800px]">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex flex-col justify-center pl-4 space-y-2">
          <h1 className="text-8xl font-regular text-white z-10 w-[600px] p-6">Collections</h1>
          <p className="text-3xl font-thin text-white z-10 w-[800px] p-6">
            Explore our diverse collections, featuring masterpieces from various artists and styles.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto p-7 pb-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-7">
          {Collections.map((collection) => (
            <div key={collection.collection_id} className="collection-item relative bg-white rounded-lg overflow-hidden shadow-lg transition-transform duration-300 transform hover:scale-105">
              <Link to={`/collection/${collection.collection_id}`}>
                ...
                <div className="absolute inset-0 bg-black bg-opacity-0 transition-opacity duration-300 ease-in-out hover:bg-opacity-40 flex items-end">
                  <h2 className="text-2xl font-thin text-white ml-4 mb-4">{collection.title}</h2>
                </div>
              </Link>
            </div>
          ))}
