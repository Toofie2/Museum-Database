...
router.put("/:id", (req, res) => {
  ...
  db.query(fetchQuery, [ReviewId], (fetchErr, fetchResult) => {
    ...
    const { customer_id, title, feedback, rating, date_posted, exhibit_id } =
      updatedReview;
    const updateQuery =
      "UPDATE Review SET customer_id = ?, title = ?, feedback = ?, rating = ?, date_posted = ?, exhibit_id = ? WHERE review_id = ?";

    db.query(
      updateQuery,
      [customer_id, title, feedback, rating, date_posted, exhibit_id, ReviewId],
