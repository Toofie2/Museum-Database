...
router.post("/", (req, res) => {
  ...
  db.query(
    insertQuery,
    [title, description, room_id, image_path, is_active],
    (err, result) => {
      if (err) {
        if (err.code === "ER_DUP_ENTRY") {
          return res
            .status(400)
            .json({ message: "Collection ID already exists" });
        }
        return res.status(500).json({ error: err.message });
      }
      res.status(201).json({
...
router.put("/:id", (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      ...
      const { title, description, room_id, image_path, is_active } =
        updatedCollection;
      ...
      db.query(
        updateQuery,
        [title, description, room_id, image_path, is_active, collectionId],
        (err) => {
          if (err) {
            return res.status(500).json({ error: err.message });
          }
...
router.delete("/:id", (req, res) => {
  ...
  db.query(
    "UPDATE Collection SET is_active = FALSE WHERE collection_id = ? AND is_active = TRUE",
    [collectionId],
    (err, result) => {
      if (err) {
        return res.status(500).json({ error: err.message });
      ...
      if (result.affectedRows === 0) {
        return res
          .status(404)
          .json({ message: "Collection not found or already inactive" });
      }
