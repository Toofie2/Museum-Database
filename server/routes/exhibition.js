...
router.get('/:id', (req, res) => {
    db.query('SELECT * FROM Exhibition WHERE exhibit_id = ?', [req.params.id], (err, results) => {
        if (err) {
            res.status(500).json({ error: err.message });
            return;
        }
...
router.post('/', (req, res) => {
    const { exhibit_id, room_id, name, description, image_path, start_date, end_date, is_active } = req.body;
        const insertQuery = 'INSERT INTO Exhibition (exhibit_id, room_id, name, description, image_path, start_date, end_date, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
        db.query(insertQuery, [exhibit_id, room_id, name, description, image_path, start_date, end_date, is_active], (err, result) => {
            if (err) {
...
router.put('/:id', (req, res) => {
    ...
    db.query(fetchQuery, [exhibitId], (fetchErr, fetchResult) => {
        ...
        const { room_id, name, description, image_path, start_date, end_date, is_active } = updatedExhibition;
        ...
        const updateQuery = `
            ...
            WHERE exhibit_id = ?
        `;
        db.query(updateQuery, [room_id, name, description, image_path, start_date, end_date, is_active, exhibitId], (updateErr, updateResult) => {
            if (updateErr) {
            ...
            res.json({ exhibit_id: exhibitId, room_id, name, description, image_path, start_date, end_date, is_active });
        });
    });
});
...
router.delete('/:id', (req, res) => {
    db.query('UPDATE Exhibition SET is_active = FALSE WHERE exhibit_id = ? AND is_active = TRUE', [req.params.id], (err, result) => {
        if (err) {
            res.status(500).json({ error: err.message });
            return;
        }
        if (result.affectedRows === 0) {
            res.status(404).json({ message: 'Exhibition not found or already inactive' });
            return;
        }
...
router.patch('/:id/reactivate', (req, res) => {
    db.query('UPDATE Exhibition SET is_active = TRUE WHERE exhibit_id = ?', [req.params.id], (err, result) => {
        if (err) {
            res.status(500).json({ error: err.message });
            return;
        }
